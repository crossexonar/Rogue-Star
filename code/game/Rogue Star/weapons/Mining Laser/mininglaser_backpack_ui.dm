/obj/item/device/continuous_cmlaser/tgui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Medigun", name)
		ui.open()

/obj/item/device/continuous_cmlaser/tgui_data(mob/user)
	//var/mob/living/H = cmlaser.current_target
	var/patientname
	var/patienthealth = 0
...
/obj/item/device/continuous_cmlaser/proc/get_examine_data()
	return list(
		"smodule" = smodule ? list("name" = smodule.name, "range" = cmlaser.beam_range, "rating" = smodule.get_rating()) : null,
...
/obj/item/device/continuous_cmlaser/tgui_act(action, params, datum/tgui/ui)
	if(..())
		return TRUE
	...
	switch(action)
		...
		if("cancel_healing")
			if(cmlaser?.busy)
				cmlaser.busy = MEDIGUN_CANCELLED
				return TRUE

		if("toggle_maintenance")
			maintenance = !maintenance
			reattach_cmlaser(ui.user)
			return TRUE
...
/obj/item/device/continuous_cmlaser/AltClick(mob/user)
	. = ..()
	cell_eject()
/obj/item/device/continuous_cmlaser/proc/cell_eject()
	if(!ccell)
		return FALSE
	charging = FALSE
	...
	update_icon()
	return TRUE
/obj/item/device/continuous_cmlaser/ui_action_click()
	. = ..()
	if(!cmlaser)
		return
	tgui_interact(usr)
